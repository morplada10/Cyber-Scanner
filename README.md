...
* **Backend (Python):** This is where the processing happens. Our Python script analyzes the text against security rules and decides if the email is dangerous, suspicious, or safe.
...
* **Keyword Analysis:** words that are high-pressure and phishing-related terms (i.e. *Urgent, Credit Card, Password,*).
...
The scanner doesn't just look for "bad words." It checks the relationship between the sender and the content. For example, a "Payment" request is flagged as high-risk if it comes from a domain the user has never interacted with before (Unknown Domain).

## Scoring Logic (The Decision Making)
I designed a weighted scoring system to evaluate risk based on specific signals:
* **The "Red Flag" (75 Points):** If an email contains both a high-pressure keyword (like "immediately" or "verify now") AND a link, it's flagged as **Danger**. This is because the combination of urgency and a call-to-action is a classic phishing tactic.
...
* **Smart Context Analysis (+45 Points):** This is a unique feature that detects "Behavioral Anomalies." If an email contains payment-related keywords (like Invoice) but the sender's domain is NOT in the Trusted Domains list, it triggers a high-risk warning. This protects users from sophisticated fraud where the language seems legitimate but the source is unknown.
...
* **URL Reputation:** Integration with APIs to check if links lead to known malicious sites.
* **LLM Integration (AI):** Instead of relying on a fixed list of keywords, I would integrate a Large Language Model. This would allow the system to understand the intent and content of the email and recognize suspicious patterns and thousands of "danger words" automatically, even if they aren't on my manual list.
